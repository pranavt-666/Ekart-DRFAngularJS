...
class ProductView(ModelViewSet):
    ...
    def get_reviews(self, request, *args, **kwargs):
        pid = kwargs.get('pk')
        product = models.Product.objects.get(id=pid)
        prod_review = models.Review.objects.filter(product_name=product)
        # print('ddddd',prod_review)
        serializing = serializers.ProductReviewSerializer(prod_review, many=True)
        print(serializing)

        # if serializing.is_valid():
        return Response(data=serializing.data)
        # else:
        #     return Response(data=serializing.errors)
